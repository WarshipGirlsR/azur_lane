...
local img3_2 = require 'BaseOperate__maps__img3_2'
local map3_2 = {}

-- 获取地图采样位置。由于地图可能超出一屏，所以这里可以定义多个采样位置。每次扫描都会对每个采样位置进行扫描
-- 标志位为地图四个角。每个采样位置只需定义一个角的坐标即可。
-- 还需要定义每个采样位置的地图矩阵与屏幕坐标的映射关系
map3_2.getCheckpositionList = function()
  local list = {
    {
      leftTop = { 348, 478 },
      rightTop = nil,
...
-- 获取地图棋盘和相关数据
map3_2.getMapChessboard = function()
  return {
    width = 8,
    height = 4,
    obstacle = {
...
map3_2.getMapPosition = function(currentPosition)
  return mapBase.getMapPosition(img3_2, currentPosition)
end

map3_2.getMoveVector = function(currentPosition, targetPosition)
  return mapBase.getMoveVector(img3_2, currentPosition, targetPosition)
end

map3_2.moveMapToCheckPosition = function(moveVector)
  return mapBase.moveMapToCheckPosition(img3_2, moveVector)
end

map3_2.scanMap = function(targetPosition, mapChessboard)
  return mapBase.scanMap(img3_2, targetPosition, mapChessboard)
end

map3_2.moveToPoint = function(targetPosition, point)
  return mapBase.moveToPoint(img3_2, targetPosition, point)
end

map3_2.checkMoveToPointPath = function(mapChessboard, start, target)
  return mapBase.checkMoveToPointPath(map3_2, mapChessboard, start, target)
end

map3_2.findClosestEnemy = function(mapChessboard)
  return mapBase.findClosestEnemy(img3_2, mapChessboard)
end

map3_2.getRandomMoveAStep = function(mapChessboard)
  return mapBase.getRandomMoveAStep(img3_2, mapChessboard)
end
