...
local img2_2 = require 'BaseOperate__maps__img2_2'
local map2_2 = {}

-- 获取地图采样位置。由于地图可能超出一屏，所以这里可以定义多个采样位置。每次扫描都会对每个采样位置进行扫描
-- 标志位为地图四个角。每个采样位置只需定义一个角的坐标即可。
-- 还需要定义每个采样位置的地图矩阵与屏幕坐标的映射关系
map2_2.getCheckpositionList = function()
  local list = {
    {
      leftTop = { 436, 498 },
      rightTop = nil,
...
-- 获取地图棋盘和相关数据
map2_2.getMapChessboard = function()
  return {
    width = 7,
    height = 5,
    obstacle = {
...
map2_2.getMapPosition = function(currentPosition)
  return mapBase.getMapPosition(img2_2, currentPosition)
end

map2_2.getMoveVector = function(currentPosition, targetPosition)
  return mapBase.getMoveVector(img1_1, currentPosition, targetPosition)
end

map2_2.moveMapToCheckPosition = function(moveVector)
  return mapBase.moveMapToCheckPosition(img1_1, moveVector)
end

map2_2.scanMap = function(targetPosition, mapChessboard)
  return mapBase.scanMap(img2_2, targetPosition, mapChessboard)
end

map2_2.moveToPoint = function(targetPosition, point)
  return mapBase.moveToPoint(img2_2, targetPosition, point)
end

map2_2.checkMoveToPointPath = function(mapChessboard, start, target)
  return mapBase.checkMoveToPointPath(map2_2, mapChessboard, start, target)
end

map2_2.findClosestEnemy = function(mapChessboard)
  return mapBase.findClosestEnemy(img2_2, mapChessboard)
end

map2_2.getRandomMoveAStep = function(mapChessboard)
  return mapBase.getRandomMoveAStep(img2_2, mapChessboard)
end
