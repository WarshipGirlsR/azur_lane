...
local img6_4 = require 'BaseOperate__maps__img6_4'
local map6_4 = {}

-- 获取地图采样位置。由于地图可能超出一屏，所以这里可以定义多个采样位置。每次扫描都会对每个采样位置进行扫描
-- 标志位为地图四个角。每个采样位置只需定义一个角的坐标即可。
-- 还需要定义每个采样位置的地图矩阵与屏幕坐标的映射关系
map6_4.getCheckpositionList = function()
  local list = {
    {
      leftTop = nil,
      rightTop = { 1712, 490 },
...
-- 获取地图棋盘和相关数据
map6_4.getMapChessboard = function()
  return {
    width = 8,
    height = 6,
    obstacle = { { 1, 7 }, { 1, 8 }, { 2, 8 }, { 3, 2 }, { 5, 5 } },
...
map6_4.getMapPosition = function(currentPosition)
  return mapBase.getMapPosition(img6_4, currentPosition)
end

map6_4.getMoveVector = function(currentPosition, targetPosition)
  return mapBase.getMoveVector(img6_4, currentPosition, targetPosition)
end

map6_4.moveMapToCheckPosition = function(moveVector)
  return mapBase.moveMapToCheckPosition(img6_4, moveVector)
end

map6_4.scanMap = function(targetPosition, mapChessboard)
  return mapBase.scanMap(img6_4, targetPosition, mapChessboard)
end

map6_4.moveToPoint = function(targetPosition, point)
  return mapBase.moveToPoint(img6_4, targetPosition, point)
end

map6_4.checkMoveToPointPath = function(mapChessboard, start, target)
  return mapBase.checkMoveToPointPath(map6_4, mapChessboard, start, target)
end

map6_4.findClosestEnemy = function(mapChessboard)
  return mapBase.findClosestEnemy(img6_4, mapChessboard)
end

map6_4.getRandomMoveAStep = function(mapChessboard)
  return mapBase.getRandomMoveAStep(img6_4, mapChessboard)
end
